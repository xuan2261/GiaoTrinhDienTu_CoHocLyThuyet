...
(function(root, factory) {
  ...
  function ensureDefs(svg) {
    ...
    if (!defs) return;
    const ids = svg.__sim2Ids;
    const filt = el('filter', { id: ids.shadow, x: '-20%', y: '-20%', width: '140%', height: '140%' });
  ...
  function depthFill(tf, opts, fallback) {
    if (opts.gradient && tf.svgIds && tf.svgIds.gradients[opts.gradient]) {
      return `url(#${tf.svgIds.gradients[opts.gradient]})`;
    }
  ...
  function createSvg(width, height) {
    ...
    svg.__sim2Ids = { shadow: `${prefix}-shadow`, gradients: {} };
    const pal = (typeof window !== 'undefined' && window.Sim2Palette) || {};
    for (const key in pal) svg.__sim2Ids.gradients[key] = `${prefix}-grad-${key}`;
    ensureDefs(svg);
    return svg;
  }
  ...
  function circle(tf, center, r, opts) {
    ...
    const node = el('circle', {
      ...
      fill: depthFill(tf, opts, 'none'),
      stroke: opts.stroke || '#333',
      'stroke-width': opts.width != null ? opts.width : (opts.depth ? 2.5 : 2)
    });
    if (opts.depth && tf.svgIds) node.setAttribute('filter', `url(#${tf.svgIds.shadow})`);
  ...
  function poly(tf, points, opts) {
    ...
    const node = el(opts.closed ? 'polygon' : 'polyline', {
      points: pts,
      fill: depthFill(tf, opts, (opts.closed ? 'rgba(80,140,255,0.15)' : 'none')),
      stroke: opts.stroke || '#333',
      'stroke-width': opts.width != null ? opts.width : (opts.depth ? 2.5 : 2)
    });
    if (opts.depth && tf.svgIds) node.setAttribute('filter', `url(#${tf.svgIds.shadow})`);
