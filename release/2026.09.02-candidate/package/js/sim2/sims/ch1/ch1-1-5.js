  ...
  Reg.register('ch1-1-5', function(container) {
    ...
    function render2() {
      ...
      panel.setReadout([
        ...
        { key: 'Mo', label: 'Mo:', value: red.Mo.toFixed(1) + ' N·m' }
      ]);
    ...
    const handles = forces.map((f, index) => {
      ...
      return shell.addHandle(tip0, {
        fill: Pal.handle,
        a11y: { label: `Đầu vectơ lực ${index + 1}`, axis: 'both' },
        keyboardStep: { x: VIS, y: VIS },
        onDrag(wp) {
          const other = forces[index === 0 ? 1 : 0].F;
          const otherVector = { x: other.fx * VIS, y: other.fy * VIS };
          const clampVector = (value, base, otherValue) => Math.max(
            Math.max(-3.5 - base, -3.5 - otherValue),
            Math.min(Math.min(3.5 - base, 3.5 - otherValue), value - base)
          );
          f.F.fx = clampVector(wp.x, f.r.x, otherVector.x) / VIS;
          f.F.fy = clampVector(wp.y, f.r.y, otherVector.y) / VIS;
          render2();
        }
      });
    });
