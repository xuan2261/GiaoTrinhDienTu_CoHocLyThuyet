  ...
  function create(opts) {
    ...
    const shell = root.Sim3Shell.create({
      ...
      setup({ THREE, scene, camera, labels }) {
        THREERef = THREE;
        if (root.Sim3VisualKit) root.Sim3VisualKit.setCamera(camera, { x: 4.55, y: 3.45, z: 6.95 }, { x: -0.18, y: 0.08, z: 0.03 });
        const mat = new THREE.MeshStandardMaterial({ color: 0x6d28d9, roughness: 0.46, metalness: 0.08 });
        disk = new THREE.Mesh(new THREE.CylinderGeometry(diskRadius, diskRadius, 0.22, 64), mat);
        disk.rotation.x = Math.PI / 2;
        rim = new THREE.Mesh(
          new THREE.TorusGeometry(diskRadius + 0.02, 0.03, 10, 72),
          new THREE.MeshStandardMaterial({ color: 0x4c1d95, roughness: 0.4 })
        );
        orbit = new THREE.Mesh(
          new THREE.TorusGeometry(diskRadius, 0.012, 8, 72),
        ...
        for (let i = 0; i < 16; i++) {
          ...
          tickMark.position.set(0.98 * Math.cos(a), 0.98 * Math.sin(a), 0.14);
        ...
        scene.add(grid);
        labels.add('point-m', 'M', () => marker.position, root.Sim3VisualKit && root.Sim3VisualKit.labelOffset('point', { dx: 18, dy: -8 }));
        labels.add('omega', 'ω', () => omegaArrow.position);
        labels.add('velocity', 'v', () => vArrow.position, root.Sim3VisualKit && root.Sim3VisualKit.labelOffset('vector', { dx: 26, dy: -10 }));
      }
    });
    ...
    function setState(state) {
      ...
      const r = diskRadius, phi = state.phi || 0;
      ...
      vArrow.scale.y = root.Sim3VisualKit ? root.Sim3VisualKit.vectorScale(state.omega, { min: 0.54, max: 1.05, factor: 0.46 }) : Math.max(0.54, Math.min(1.05, Math.abs(state.omega || 0) * 0.46));
      shell.setState(state);
      root.__SIM3_DEBUG__ = root.__SIM3_DEBUG__ || {};
      root.__SIM3_DEBUG__['ch2-2-2'] = Object.assign({
        updatedAt: tick,
        visualMetrics: root.Sim3VisualKit && root.Sim3VisualKit.visualMetrics({
          diskRadius,
          diskScaleRole: 'reduced',
          tangentMarginTargetPx: 32,
          vectorScaleMax: 1.05,
          labelSeparationTargetPx: 12,
          axisRole: 'visible-secondary'
        })
      }, state);
    }
