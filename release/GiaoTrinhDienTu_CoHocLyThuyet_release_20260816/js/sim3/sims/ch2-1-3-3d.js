(function(root) {
  ...
  function create(opts) {
    ...
    const pathScale = 0.54;
    const shell = root.Sim3Shell.create({
      host: opts.host,
      referenceEl: opts.referenceEl,
      label: 'Tiếp pháp tuyến và bán kính cong 3D',
      onFallback: opts.onFallback,
      setup({ THREE, scene, camera, labels }) {
        THREERef = THREE;
        root.Sim3VisualKit.setCamera(camera, { x: 3.6, y: 3.0, z: 5.2 }, { x: 0.15, y: 0.15, z: 0 });
        ...
        scene.add(new THREE.Line(
          ...
          new THREE.LineBasicMaterial({ color: root.Sim3VisualKit.colors.axis })
        ));
        ...
        centerDot = new THREE.Mesh(
          new THREE.SphereGeometry(0.07, 16, 12),
          root.Sim3VisualKit.material(THREE, 'moment')
        );
        circle = new THREE.Mesh(
          new THREE.TorusGeometry(1, 0.02, 10, 112),
          root.Sim3VisualKit.material(THREE, 'moment', { transparent: true, opacity: 0.78, emissive: 0x120020 })
        );
        circle.rotation.x = Math.PI / 2;
        scene.add(point, tangentArrow, normalArrow, radiusLine, centerDot, circle);
    ...
    function setState(state) {
      tick += 1;
      const p = state.point || { x: 0, y: 0 };
      const t = state.tangent || { x: 1, y: 0 };
      const n = state.normal || { x: 0, y: 1 };
      const radius = Math.max(0.1, Math.min(12, state.radius || 1));
      const pos = { x: p.x * pathScale, y: 0.14, z: p.y * pathScale };
      point.position.set(pos.x, pos.y, pos.z);
      setArrow(THREERef, tangentArrow, pos, { x: t.x, y: 0, z: t.y }, 0.9);
      setArrow(THREERef, normalArrow, pos, { x: n.x, y: 0, z: n.y }, 0.82);
      const center = { x: (p.x + n.x * radius) * pathScale, y: 0.09, z: (p.y + n.y * radius) * pathScale };
      circle.position.set(center.x, center.y, center.z);
      centerDot.position.set(center.x, center.y + 0.03, center.z);
      circle.scale.setScalar(Math.max(0.2, radius * pathScale));
      root.Sim3Primitives.setCylinderBetween(THREERef, radiusLine, pos, center);
      ...
      root.__SIM3_DEBUG__['ch2-1-3'] = Object.assign({}, state, {
        ...
        visualMetrics: root.Sim3VisualKit.visualMetrics({
          frenetFrame: 'tangent-normal-radius',
          osculatingCircleRole: 'secondary-guide',
          labelClusterReduced: true,
          labelClusterStrategy: 'frenet-lanes-separated',
          labelSeparationTargetPx: 18,
          osculatingCircleContrast: 'enhanced',
          osculatingCircleOpacityMin: 0.78,
          radiusGuideStrokeMin: 0.022,
          radiusLabelAttachmentPxMax: 24
        })
      });
    }
