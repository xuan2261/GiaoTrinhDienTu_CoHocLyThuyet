...
(function() {
  ...
  function injectBookmarkBtn() {
    ...
    function renderBookmarkState(bookmarked) {
      const label = bookmarked ? 'Bỏ đánh dấu trang này' : 'Đánh dấu trang này';
      btn.textContent = bookmarked ? '🔖' : '📑';
      btn.title = label;
      btn.setAttribute('aria-label', label);
      btn.setAttribute('aria-pressed', String(bookmarked));
    }
    renderBookmarkState(isBookmarked);
    btn.addEventListener('click', () => {
      const bm2 = getBookmarks();
      const idx = bm2.indexOf(pageId);
      if (idx >= 0) bm2.splice(idx, 1);
      else bm2.push(pageId);
      renderBookmarkState(idx < 0);
      saveBookmarks(bm2);
    });
