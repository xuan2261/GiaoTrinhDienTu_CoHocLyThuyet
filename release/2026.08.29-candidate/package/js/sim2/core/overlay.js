...
(function(root, factory) {
   ...
   */
  function createOverlay(parent, tf, logicalWidth, logicalHeight) {
    ...
    function position(div) {
      ...
      let left = s.x * scaleX, top = s.y * scaleY;
      div.style.left = left + 'px';
      div.style.top = top + 'px';
      div.style.transform = anchorTransform(div.__anchor);
      const bounds = layer.getBoundingClientRect();
      const rect = div.getBoundingClientRect();
      if (rect.left < bounds.left) left += bounds.left - rect.left;
      if (rect.right > bounds.right) left -= rect.right - bounds.right;
      if (rect.top < bounds.top) top += bounds.top - rect.top;
      if (rect.bottom > bounds.bottom) top -= rect.bottom - bounds.bottom;
      div.style.left = left + 'px';
      div.style.top = top + 'px';
    }
    ...
    function resize(displayWidth, displayHeight) {
      scaleX = displayWidth / logicalWidth;
      scaleY = displayHeight / logicalHeight;
      reflow();
    }
