"""Fix check_executable_harness Phase 05 harness indentation in smoke_simulation_runtime.py"""
...
next_func = content.find('\ndef check_activity_progress_schema_guard')
print(f"Function: lines {content[:func_start].count(chr(10))+1}-{content[:next_func].count(chr(10))+1}")

# Find the start of the old Phase 05 block (line 531, "if phase05_routes:")
old_phase05_start = content.find('        if phase05_routes:')
# Find the start of the Phase 01 else (after the old Phase 05 block)
old_phase01_else = content.find('\n    else:\n        # Phase 01 harness')
print(f"Old Phase 05 block: lines {content[:old_phase05_start].count(chr(10))+1}-{content[:old_phase01_else].count(chr(10))+1}")

# The new correct Phase 05 block (with proper 8-space indent = 2 levels of 4)
new_phase05 = '''        if phase05_routes:
...
new_content = content[:old_phase05_start] + new_phase05 + content[old_phase01_else:]
