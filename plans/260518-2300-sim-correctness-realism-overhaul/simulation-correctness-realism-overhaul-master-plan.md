...
| 02 | [RC1 Handle Body Anchor Coupling](./phase-02-rc1-handle-body-anchor-coupling.md) | Complete | Engine fail-loud `resolveHandles`, `legacyHandles` deleted entirely, ch1-1-3/ch1-2-1/ch1-1-8 explicit branches. Browser invariant `@rc1-handle-anchor` GREEN across 5 RC1 routes — no `legacy-primary` fallback exists. Per-renderer `getAnchor` field considered YAGNI under current contract. |
| 03 | [RC4 Spring And Cable Base Coupling](./phase-03-rc4-spring-cable-base-coupling.md) | Complete | `P.spring` accepts `{ anchor, wallAnchor, gap }` and always emits `springAnchor:` mark when drawn. Browser invariant `@rc4-spring-base` GREEN across 4 RC4 routes. |
...
| 08 | [RC2 RC6 Animation Density And Theme Parity](./phase-08-rc2-rc6-animation-density-theme-parity.md) | Complete | Impulse flash (ch3-6-2), preset gallery (ch1-2-3/1-3/2-1), ch2-1-1 trail buffer, ch3-3-1 spring autoplay (resume() rAF re-arm fix), and RC6 theme-aware palette via `SimCore.color()` + `SimCore.PALETTE_SOURCE` all shipped. |
