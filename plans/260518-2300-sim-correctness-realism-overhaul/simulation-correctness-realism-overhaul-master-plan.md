...
| 08b | [Accessibility ARIA Keyboard And Reduced Motion](./phase-08b-accessibility-aria-keyboard.md) | Partial | Engine: `setupA11yOverlay` (Vietnamese ARIA + keyboard nudge/shift/escape + aria-live polite) and `attachReducedMotion` (`lab.prefersReducedMotion` flag) shipped. CSS for overlay layer + sr-only + downstream consumers (preset tween, trail fade, autoplay skip) pending. |
