...
| Interactive simulation expansion | Done: 58/58 P1 routes have canonical registry contracts; 52 learner-facing routes mount `.sim-lab`, while Section VII `BÀI TẬP` pages are content-only |
...
| Professional simulation lab shell | Done: 58/58 canonical routes have lab contracts; 52 learner-facing routes dùng `.sim-lab` shell, manifest gate, responsive browser checks, và route identity gate |
...
| Simulation canvas overlay cleanup | Done: 52 learner-facing mountable routes block formula/value DOM in `.sim-lab-overlay`; right inspector owns formula/readout UI; `@overlay-contract`, browser, visual, semantic, and release gates pass |
| Professional direct interaction kernel | Done: 52 learner-facing mountable routes có direct drag/readout stability audit, touch gesture guard, keyboard nudge, autoplay-safe pause, và listener cleanup gate |
...
| Simple lab shell baseline | Done: 58-route objective/interaction manifest, 52-route learner-facing DOM shell (header/readout-cards/hint), readout cards, reset/play-pause controls, no checkpoint/assessment runtime |
...
| Phase 09 canvas evolution cleanup | Done (2026-05-21, updated 2026-05-22): concept routes use `scene.static`/`tickWithoutButton`, misleading Play is suppressed, static/animated canvas labels are exact, silent ticks survive drag; engine-time sweep covers 52 learner-facing routes / 0 defects / 0 drift |
...
| Rich animated full-physics rebuild (Phases 01-06) | **Done**: foundation architecture — Physics (Vec2/RigidBody/Spring/World), Scene (SceneNode/BodyNode/ArrowNode), Render (Primitives/Effects/Camera), Interaction (HandleManager/Gesture/Guide/Nudge), Animation (Animator/Timeline/Interpolator/Tween/PlaybackUI); active registry covers 58 P1 routes from coverage matrix |
...
| P3 | Done | Professional simulation labs | 58/58 canonical route contracts, 52 learner-facing route mounts, direct drag, readout cards, reset/play-pause, runtime smoke, browser QA pass |
...
| P9 | Done | DeCuong simulation full rebuild | Phase 00-12 complete; final review fixes landed for canonical route contracts; final release gate pass with 58 registered / 52 mountable split |
