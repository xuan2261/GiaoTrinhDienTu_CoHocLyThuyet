...
| QA harness | Đã có `npm run test:quiz`, `npm run test:quiz:browser`, `npm run test:sim:unit`, `npm run test:sim:quality`, `npm run test:sim:semantic`, `npm run test:sim:renderer-contract`, `npm run test:sim:browser`, `npm run test:sim:visual-quality`, `npm run test:sim:release`, `npm run test:sim:browser:install`, `npm run test:sim:browser:baseline`, `npm run test:sim:browser:route-mount`, `npm run test:sim:scene-identity`, `npm run test:sim:correctness`, `npm run test:sim:correctness:browser`; `test:quiz` khóa 100-question banks và bundle freshness, `test:quiz:browser` khóa all/random rendering |
