...
| Sim3 optional Three.js pilot | Done (updated 2026-06-09): 10 optional routes `ch1-1-5`, `ch1-5-3`, `ch2-1-3`, `ch2-2-2`, `ch2-3-2`, `ch2-4-4`, `ch2-5-3`, `ch3-1-3`, `ch3-5-3`, `ch3-6-2`; offline Three.js vendored at `lib/three/three.umd.min.js`, Sim2 stays default, WebGL fail falls back to 2D |
...
| QA harness | Đã có `npm run test:content`, `npm run test:quiz`, `npm run test:quiz:browser`, `npm run test:sim:unit`, `npm run test:sim:quality`, `npm run test:sim:semantic`, `npm run test:sim:renderer-contract`, `npm run test:sim:browser`, `npm run test:sim:visual-quality`, `npm run test:sim:release`, `npm run test:sim:browser:install`, `npm run test:sim:browser:baseline`, `npm run test:sim:browser:route-mount`, `npm run test:sim:scene-identity`, `npm run test:sim:correctness`, `npm run test:sim:correctness:browser`; `test:content` khóa author page và cleanup route review Section VII, `test:quiz` khóa 100-question banks và bundle freshness, `test:quiz:browser` khóa all/random rendering |
