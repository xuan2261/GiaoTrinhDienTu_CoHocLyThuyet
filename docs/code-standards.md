...
- Mount: `window.SIM_MAP[pageId] -> factory(container) -> { dispose }`.
- `dispose()` phải hủy listener, RAF và DOM do simulation tạo.
- Sim2 bắt đầu paused khi có playback; hỗ trợ `prefers-reduced-motion`.
- Nhãn dùng HTML overlay, geometry dùng SVG; canvas underlay chỉ opt-in cho trail/field.
- Màu semantic dùng `Sim2Palette`/`--sim-c-*`, không rải hex tùy ý.
- Sim3 phải fallback về Sim2 với thông báo tiếng Việt khi WebGL lỗi.
...
`theme`, `fontZoom`, `quizScores`, `chlyt_progress`, `chlyt_bookmarks`, `chlyt_notes`. Không đổi key nếu chưa có migration rõ.
