...
| Accessibility evidence | Shell/search/quiz/PDF/Sim2/Sim3 có landmarks, keyboard, focus, reflow và contrast contracts; manual/independent review vẫn bắt buộc |
...
- Quiz hỗ trợ all/random có seed, stable IDs/LO metadata, semantic radio controls và khôi phục attempt cục bộ.
...
- Requirement traceability maps legal requirements through provisional learning outcomes to canonical content, quiz, Sim2 and evidence records; formal acceptance remains role-reviewed, not inferred from technical validation.
- Accessibility baseline phải tách evidence tự động khỏi kiểm tra thủ công, giữ `file://` parity và không được diễn giải thành chứng nhận WCAG 2.2 AA.
...
5. `npm run test:accessibility` phải pass khi chốt Phase 8; browser zoom, screen reader, text spacing và scientific visualization equivalence vẫn cần manual review độc lập.
6. `python tools\audit.py`, cùng strict image/equation gates khi publish, không báo lỗi thật.
7. Generated files chỉ được cập nhật qua pipeline.
...
| Pass automation bị diễn giải quá mức | Baseline ghi rõ phạm vi/exclusion, trạng thái run/manual riêng và cấm blanket conformance claim |
